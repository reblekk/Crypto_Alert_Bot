                    ...
                    if (__status == "active") {
                       // console.log("here")
                        if (__upndown == "up" || __upndown == "UP" || __upndown == "Up" || __upndown == "uP") {
                            if (__price >= __com_price && __status == "active" ) {
...
bot.onText(/\/list/, function(msg, match) {
    ...
    for (i = 0; i < __detect; i++) {
        let john = db.get('posts')
            .find({ id: i })
            .value()
            //console.log(john.chatId)
        if (john.chatId == chatId && john.status == "active") {
            __storage = __storage + john.ticker + '/' + john.compare + ' ' + john.com_price + ' ' + john.upndown + '\n'
        }

    }
    //console.log(__storage)
    if (__storage == '') {
        bot.sendMessage(chatId, "you havent set any list yet.")
    } else {
        bot.sendMessage(chatId,"active alert list:\n" + __storage)
    }
})
...
bot.onText(/\/commands/, function(msg, match) {
    ...
    bot.sendMessage(chatId, "/set to set alerts \nEx 1. /set bnb usd 200 up \nThis will alert you when bnb is above $200\nEx 2. /set bnb usd 100 down \nThis will alert you when price of bnb is under $100\nYou can set alert for other tokens or any pair too like this. Any questions/confusions send me a message @fahimlawls\n/p for price\n/list for list of your active alerts")
...
bot.onText(/\/start/, function(msg, match) {
    ...
    bot.sendMessage(chatId, "/commands for the commands")
